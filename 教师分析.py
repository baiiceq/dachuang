...
with st.sidebar:
    st.header("数据上传")
    uploaded_file = st.file_uploader("上传学生数据（JSON格式）", type="json")
    df = load_data(uploaded_file)

    st.divider()
    st.markdown("**数据字段说明：**")
    st.markdown(
        "- weak_points: 薄弱知识点列表\n- study_period: 学习时段（早晨/下午/晚上）\n- question_count: 平均提问数量")

# 显示原始数据
with st.expander("🔍 点击查看原始数据", expanded=False):
...
def analyze_student_clusters(df):
    st.header("👥 学生群体分析（优化版）", divider="rainbow")
...
if __name__ == "__main__":
    weak_result = analyze_weak_points(df)
    behavior_result = analyze_learning_behavior(df)
    cluster_result = analyze_student_clusters(df)
    network_result = analyze_knowledge_network(df)
    # 构建分析结果
    analysis_results = {
        "top_weak_points": weak_result['top_weak_points'],
        "weak_distribution": weak_result['weak_distribution'],
        "study_period_dist": behavior_result['period_dist'],
        "question_stats": behavior_result['question_stats'],
        "cluster_summary": {
            "total_clusters": cluster_result['cluster_stats']['total_clusters'],
            "cluster_dist": cluster_result['cluster_stats']['size_distribution'],
            "silhouette": cluster_result['cluster_stats']['silhouette_score'],
            "features": cluster_result['cluster_stats']['features']
        },
        "knowledge_network": {
            "top_combinations": network_result['top_combinations'],
            "network_density": network_result['network_density'],
            "isolated_points": network_result.get('isolated', [])
        }
    }
